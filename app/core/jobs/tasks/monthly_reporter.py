...
async def generate_report(user_id):
    ...
    context = {
        ...
        'highlights': [task for task in tasks if task.category == "highlights"],  # Separate highlights category
...
async def send_monthly_report(to_email=None, force=False, job_id=None, user_id=None):
    ...
    base_job_id = 'monthly_reporter'  # Always use base job ID for email config
    ...
    async with JobExecutionLock(actual_job_id, timeout_minutes=30) as lock:
        if not lock.acquired:
            logger.info(f"Monthly reporter job {actual_job_id} is already running, skipping this execution")
            return {
                'job_id': actual_job_id,
                'status': 'skipped',
        ...
        try:
            ...
            if not force and (today_ist.weekday() != 4 or not is_last_friday(today_ist)):
                ...
                await tracker.log("INFO", f"Current day: {today_ist.strftime('%A')}, Is last Friday: {is_last_friday(today_ist)}")
